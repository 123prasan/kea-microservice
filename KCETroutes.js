...
router.get('/Oauth/login', async (req, res) => {
    try {
        console.log("🔥", process.env.KCET_GOOGLE_CLIENT_ID);
        console.log("🔥", process.env.KCET_GOOGLE_CLIENT_SECRET);
        const host = req.get('host');
        const protocol = host.includes('localhost') ? 'http' : 'https';
        const redirectUri = `${protocol}://${host}/kcet/Oauth/login`;

        const oauth2Client = new OAuth2Client(
            process.env.KCET_GOOGLE_CLIENT_ID,
            process.env.KCET_GOOGLE_CLIENT_SECRET,
            redirectUri
        );
        ...
        const ticket = await oauth2Client.verifyIdToken({
            ...
            audience: process.env.KCET_GOOGLE_CLIENT_ID
        });
        ...
        res.send(`
            ...
            <head>
                ...
                <script>
                    ...
                    window.location.href = 'https://vidyari.com/kcet';
            </head>
            <body>
        ...
        res.status(500).send(`
            ...
            <body style="font-family: sans-serif; text-align: center; margin-top: 50px;">
                ...
                <p>\${err.message}</p>
                <a href="https://vidyari.com/kcet/login">Back to Sign In</a>
            </body>
            </html>
        `);
    }
...
router.get('/api/predict', async (req, res) => {
    ...
    try {
        ...
        const cacheKey = `predict_${userRank}_${categoriesList.join('|')}_${course_name || 'all'}_${course_category || 'all'}`;
        const cachedResult = predictCache.get(cacheKey);
        if (cachedResult) {
        ...
        // Save to cache
        predictCache.set(cacheKey, predictions);
